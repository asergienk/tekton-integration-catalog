...
|packageName|An OLM package name present in the fragment or leave it empty so the step will determine the default package name as described above.|If there is only one 'olm.package', it's name is returned. If there are multiple 'olm.package' entries, the package with the highest bundle version in its 'defaultChannel' is selected|false|
|channelName|An OLM channel name or leave it empty so the step will determine the default channel name as described above.|The defaultChannel name for a given package.|false|
